...
function splitText(text, maxLen = 480, overlap = 80) {
    const paras = text
        .split(/\n{2,}/)
        .map(p => p.trim())
...
async function seed() {
    ...
    console.log('╚══════════════════════════════════════════════╝');
    console.log(`\nMaterias a procesar: ${Object.keys(subjects_content_1.SUBJECT_CONTENTS).length}`);
    console.log('Modelo: text-embedding-004 (768 dimensiones)\n');
    let totalChunks = 0;
    let totalSubjects = 0;
    for (const [subjectId, subject] of Object.entries(subjects_content_1.SUBJECT_CONTENTS)) {
